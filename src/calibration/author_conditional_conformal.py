...
from src.calibration.author_conformal import AuthorSplitConformalCalibration
from src.calibration.author_utils import compute_threshold
from src.calibration.author_utils import append_result_to_csv
from src.calibration.author_utils import split_group

CORRECT_ANNOTATIONS = ["S"]


class AuthorGroupConditionalConformal(AuthorSplitConformalCalibration):
    ...
    def compute_factual_results(self, data, alphas, a, calibrate_range=0.5):
        ...
        for method in self.confidence_method:
            ...
            for alpha in tqdm(alphas, desc=f"Computing factual results for {method}"):
                ...
                for _ in range(self.runs):
                    ...
                    for group in groups:
                        ...
                        correctness_per_group[group].append(fraction_correct[group])
        ...
        for grp, grp_results in per_group_results.items():
            csv_name = os.path.join(
                self.result_dir, f"{self.dataset_name}_{grp}_factual_correctness.csv"
            )
